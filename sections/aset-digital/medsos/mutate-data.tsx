...
const MedsosActions = ({ isEdit }: { isEdit?: boolean }) => {
  ...
  const rightMedsosData = MedsosData.slice(2);
    ...
    <Card className="w-full border-primary dark:border-muted-foreground">
      <CardContent className="p-3 flex gap-4">
        <div className="w-full mt-[6px]">
          ...
          {leftMedsosData.map((field, idx) => (
            <div
              key={idx}
              // className={`transition-all duration-300 ease-in-out ${
              //   idx === 0 && isAktif ? "mt-[36px]" : "mt-1"
              // }`}
            >
              <InputForm
                type={field.type}
        ...
        <div className="w-full">
          ...
          {isAktif && (
            <div
              className={`mt-[22px] transition-all duration-300 ease-in-out ${
